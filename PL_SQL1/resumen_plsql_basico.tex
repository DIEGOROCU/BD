...
\begin{lstlisting}
DECLARE
  v_emp emp%ROWTYPE;  -- Estructura de tabla
...
\section*{9. Procedimientos}

\textbf{CREATE PROCEDURE}: Bloque con nombre almacenado en la BD.
...
BEGIN
  UPDATE emp SET sal=sal*(1+p_porc/100) WHERE id=p_id;
  COMMIT;
END;
/
\end{lstlisting}
...
\textbf{Ejecución}:
\begin{lstlisting}
EXECUTE actualizar_sal(101, 10);
-- O desde bloque:
BEGIN actualizar_sal(101, 10); END;
/
\end{lstlisting}

\textbf{Parámetros}: IN (entrada), OUT (salida), IN OUT (ambos).

\section*{10. Funciones}

\textbf{CREATE FUNCTION}: Similar a procedimiento pero retorna valor.

\begin{lstlisting}
CREATE OR REPLACE FUNCTION calc_iva(
  p_precio NUMBER
) RETURN NUMBER IS
BEGIN
...
\textbf{Uso}:
\begin{lstlisting}
SELECT nom, precio, calc_iva(precio) FROM productos;
-- O en bloque:
v_total := calc_iva(100);
\end{lstlisting}
...
\begin{lstlisting}
DECLARE
  CURSOR c IS SELECT emp_id,sal FROM emp WHERE dept=10;
BEGIN
  FOR r IN c LOOP
    UPDATE emp SET bonus=r.sal*0.1 WHERE emp_id=r.emp_id;
  END LOOP;
  COMMIT;
END;
/
...
BEGIN
  UPDATE emp SET sal=c_min WHERE sal<c_min;
